...
@endsection
@section('moreCss')
    <link rel="stylesheet" href="{{ env('APP_URL') }}/css/home/compensation.css">
@endsection()
@section('content')
    ...
    <div id="CalendarMain">
        <div id="title">
            ...
            <div class="title-text">
                <a class="selectBtn selectYear" href="javascript:" onClick="CalendarHandler.CreateSelectYear(CalendarHandler.showYearStart);">2015年</a>
        ...
        <div class="footer-text">红色标识的日期才有医保理赔产生，点击日期即可查询具体的理赔内容</div>
    </div>
    <div class="check-button" id="search" data-choose="">
        查询
    </div>
</div>
@endsection()
...
<script type="text/javascript">
    $(window).resize(function(){
        ...
        var url = "{{ url('compensation/days') }}";
        var stringM = CalendarHandler.StringModifyTime(m);
        var from = y.toString() + stringM + '00';
        var to = y.toString() + stringM + '32';
        $.ajax(url, {
            type: 'get',
            dataType: 'json',
            timeout: '120000',
            ...
            CalendarHandler.initialize(y, m, d ,data.days);
        }).fail(function (errors) {
            alert("网络错误!");
            CalendarHandler.initialize(y, m, d ,[]);
        });
        chooseDate.init();
    });
    ...
    var CalendarHandler = {
        ...
        showYearStart:2009,
        tag:0,
        initialize: function(y, m, d, givenDays) {
            $calendarItem = this.CreateCalendar(y, m, d, givenDays);
            ...
            $("#Container").css("height", "0px").css("width", "0px").css("margin-left", $("#context").width() / 2 + "px");
            $("#Container").animate({
        ...
        CreateCalendar: function(y, m, d, givenDay) {
            ...
            var stringMonth = this.StringModifyTime(nowMonth);
            var stringYear = String(nowYear);
            givenDay = givenDay ? givenDay : [];
            //生成当月的日期
            for (var i = 0; i < nowDaysNub; i++) {
                var stringDay = this.StringModifyTime(i+1);
                var specificDay = stringYear + stringMonth + stringDay;
                if (givenDay.indexOf(parseInt(specificDay))>=0){
                    $dayItem.append("<div class=\"item messageItem\"><a>" + (i + 1) + "</a></div>");
                }
                else {
        ...
        CalculateNextMonthDays: function() {
            if (this.isRunning == false) {
                ...
                var url = "{{ url('compensation/days') }}";
                var stringM = this.StringModifyTime(m);
                var from = y.toString() + stringM + '00';
                var to = y.toString() + stringM + '32';
                $.ajax(url, {
                    type: 'get',
                    ...
                    $calendarItem = _this.CreateCalendar(y, m, d, data.days);
                    $("#Container").append($calendarItem);
                    ...
                    alert("网络错误!");
                    $calendarItem = _this.CreateCalendar(y, m, d, []);
                    $("#Container").append($calendarItem);
        ...
        StringModifyTime: function(time) {
            var stringTime = String(time).length==1 ? '0'+ String(time) : String(time);
    ...
    var chooseDate = {
        init: function(){
            ...
            if (isSupportTouch) {
                $("body").on("touchstart", ".messageItem", function () {
                    var currentItem = $(".dayItem").find(".currentItem");
                    currentItem.addClass("messageItem");
                    currentItem.removeClass("currentItem");
                    $(this).addClass("currentItem");
                    $(this).removeClass("messageItem");

                    var searchDay = $(this).text();
                    var searchMonth = parseInt($(".selectMonth").text()).toString();
                    var searchYear = parseInt($(".selectYear").text()).toString();
                    ...
                    var searchTime = searchYear+searchMonth+searchDay;
                    $("#search").attr({"data-choose":searchTime});
                });
                $("body").on("touchstart", ".currentItem", function () {
                    $(this).addClass("messageItem");
                    $(this).removeClass("currentItem");
                    $("#search").attr({"data-choose":""});
                });
            }else{
                ...
                $("body").on("click", ".currentItem", function () {
                    $(this).addClass("messageItem");
                    $(this).removeClass("currentItem");
                    $("#search").attr({"data-choose":""});
                });
...
<script>
    $("#search").on("click",function () {
        var choose = $("#search").attr("data-choose");
        if (!choose || choose==""){
            alert("请选择1个日期！");
            return false;
        }
        window.location.href="{{ url('compensation/details') }}?time="+choose;
    });
</script>
@endsection()